...
function generateRecipe(event) {
  ...
  let prompt = `generate a recipe using most or all ingredients from ${ingredientsInput.value}, strictly excluding all ingredients from ${ingredientsToAvoid.value}, using the number of servings provided in ${servings}, that can be cooked within the amount of time provided in ${time} and respecting the macronutrient indications per portion from ${macros}. Please also display the recipe either in metric or imperial units, according to ${units}. Please style the recipe with simple HTML, as indicated in the context.`;
  let context =
